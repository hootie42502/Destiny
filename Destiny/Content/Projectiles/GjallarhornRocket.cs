...
namespace Destiny.Content.Projectiles
{
	public class GjallarhornRocket : ModProjectile
	{
		...
		public override void OnKill(int timeLeft) {
            ...
            /*/// Spawn a bunch of WolfPackRounds.
			for (int i = 0; i < 4; i++) {
                Vector2 v = new Vector2();
                
				v.X -= 1f;
				v.Y += 1f;
                v.RotatedByRandom(MathHelper.ToRadians(60));
				Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.position, v , ModContent.ProjectileType<Projectiles.WolfpackRound>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
				
                
			}*/

            for (int i = 0; i < 4; i++) {
    // Set the base velocity for the new projectiles (e.g., moving upwards).
    Vector2 baseVelocity = new Vector2(0f, -15f); // You can adjust the speed by changing 5f to whatever speed you'd like

    // Rotate the base velocity by a random amount between -30 and 30 degrees.
    Vector2 randomVelocity = baseVelocity.RotatedByRandom(MathHelper.ToRadians(60));

    // Spawn the new projectile with the modified velocity.
    Projectile.NewProjectile(
        Projectile.GetSource_Death(), 
        Projectile.position, 
        randomVelocity, 
        ModContent.ProjectileType<Projectiles.WolfpackRound>(), 
        Projectile.damage, 
