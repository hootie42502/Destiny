...
{
	public class KhvostovBullet1 : ModProjectile
	{
		private int ricochetCount = 0; // Counter for ricochets
		private const int MaxRicochets = 1; // Maximum number of ricochets
		...
		public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers) {
             int dropChance = Main.rand.Next(100); // 1 in 10 chance (10%)
            if (dropChance < 10) {
                // Spawn the Orb of Light item
                Item.NewItem(Projectile.GetSource_Death(), Projectile.Hitbox, ModContent.ItemType<OrbOfLight>());
            }
